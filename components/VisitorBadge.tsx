...
/**
 * Footer pill showing this week's unique visitor count.
 *
 ...
 * and reads the counter. Idempotent within a week (same IP doesn't double-
 * count). Renders nothing on first paint to avoid layout shift, then fades
 * in when the number arrives.
 */
export function VisitorBadge() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/stats/visitors"))
      .then((r) => r.json())
      .then((d: { thisWeek?: number }) => {
        if (!cancelled && typeof d.thisWeek === "number") {
          setCount(d.thisWeek);
        }
      })
      .catch(() => {
  ...
  return (
    ...
    >
      ...
      <span className="tabular-nums">
        {count.toLocaleString()} {t("visitors_this_week")}
      </span>
    </div>
  );
}
